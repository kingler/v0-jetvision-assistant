# Create Linear Issue Skill & Slash Command — Implementation Plan

> **For Claude:** REQUIRED SUB-SKILL: Use superpowers:executing-plans to implement this plan task-by-task.

**Goal:** Create a `/create-linear-issue` slash command and agent skill that creates hierarchical Linear issues (Feature/Bug > Epic > User Story > Tasks) with Given-When-Then acceptance criteria and automatic UAT generation.

**Architecture:** Two files — a slash command (`.claude/commands/create-linear-issue.md`) that provides the user-facing `/create-linear-issue` interface, and a skill (`.claude/skills/create-linear-issue/skill.md`) that documents when/how to use it. Both leverage the existing Linear MCP `create_issue`, `update_issue`, and `get_issue` tools. Accepts an optional plan document path as `$ARGUMENTS` to seed issue content from an existing plan.

**Tech Stack:** Linear MCP (create_issue, update_issue, get_issue, list_issue_labels), existing `/uat_instructions` command, Given-When-Then (Gherkin) acceptance criteria format.
...
**Files:**
- Create: `.claude/commands/create-linear-issue.md`

**Step 1: Write the slash command**

Create `.claude/commands/create-linear-issue.md` with the full command definition. This follows the exact pattern of existing commands like `linear-fix-issue.md`, `uat_instructions.md`, and `work-on-issue.md`.
...
Run: `ls -la .claude/commands/create-linear-issue.md`
Expected: File exists with reasonable size (~8KB+)

**Step 3: Commit**

```bash
git add .claude/commands/create-linear-issue.md
git commit -m "feat: add /create-linear-issue slash command for hierarchical issue creation"
```

---

## Task 2: Create the Agent Skill File

**Files:**
- Create: `.claude/skills/create-linear-issue/skill.md`

**Step 1: Create the skill directory and file**

Create `.claude/skills/create-linear-issue/skill.md` following the exact pattern of existing skills like `linear-fix-issue/skill.md` and `uat-instructions/skill.md`.
...
Run: `ls -la .claude/skills/create-linear-issue/skill.md`
Expected: File exists

**Step 3: Commit**

```bash
git add .claude/skills/create-linear-issue/
git commit -m "feat: add create-linear-issue agent skill for hierarchical issue creation"
```
...
**Files:**
- Read: `.claude/commands/create-linear-issue.md`
- Read: `.claude/skills/create-linear-issue/skill.md`
- Read: `.claude/commands/uat_instructions.md` (verify UAT integration references match)

**Step 1: Verify slash command is discoverable**

Run the skill tool to check if `create-linear-issue` appears in the skill list. In a new Claude Code session, type `/create-linear-issue` and verify it autocompletes.

**Step 2: Verify skill metadata**

Check that the skill frontmatter (`name`, `description`) matches the pattern of other skills:
- `name` field matches directory name
- `description` field starts with "Use when" for proper skill matching

**Step 3: Verify Linear MCP tool references**

Cross-check that all Linear MCP tools referenced in the command exist:
- `create_issue` — used for all hierarchy levels
- `get_issue` — used for --epic-only and --story-only validation
- `update_issue` — used for description updates
- `list_issue_labels` — used for label validation
- `create_comment` — used for UAT posting

**Step 4: Verify UAT integration path**

Confirm that the `/uat_instructions` command referenced in Step 7 of the slash command matches the actual command file at `.claude/commands/uat_instructions.md`:
- UAT document output path: `docs/uat/UAT-ONEK-{ID}-{slug}.md` ✓
- Sign-off table includes @AB and @Kham ✓
- Given-When-Then format matches ✓
...
## Task 4: Test with a Dry Run

**Step 1: Test interactive mode**

Invoke `/create-linear-issue` with no arguments and verify it:
- Asks the user what to create
- Offers Feature Request or Bug Fix choice
- Walks through the hierarchy creation flow

**Step 2: Test with plan document reference**

Invoke `/create-linear-issue .claude/plans/2026-02-12-chat-session-load.md` and verify it:
- Reads the plan document
- Extracts feature name and scope
- Suggests epics and user stories from the plan content
- Generates Given-When-Then acceptance criteria
- Offers UAT generation

**Step 3: Test --epic-only and --story-only modes**

After creating a feature, test adding:
- An additional epic: `/create-linear-issue --epic-only ONEK-XXX`
- An additional story: `/create-linear-issue --story-only ONEK-YYY`

**Step 4: Verify error handling**

Test edge cases:
- Invalid issue ID: `/create-linear-issue --epic-only ONEK-99999`
- Missing plan file: `/create-linear-issue nonexistent.md`
- Story without acceptance criteria (should auto-generate and ask for confirmation)
