...
export async function upsertQuoteFromRFQ(
  ...
  const record = {
    ...
    operator_id: (rfqFlight.operator_id ?? rfqFlight.operatorId ?? '') as string,
    ...
    status: (rfqFlight.status ?? rfqFlight.rfqStatus ?? 'received') as 'pending' | 'expired' | 'accepted' | 'rejected' | 'received' | 'analyzed',
