...
export const QuickActions = memo(function QuickActions({
  ...
  return (
    <div className={`flex flex-wrap gap-1.5 sm:gap-2 mb-3 sm:mb-4 ${className}`}>
      {actions.map((action) => (
        <Button
          key={action.label}
          variant="outline"
          ...
          disabled={disabled}
          className="text-[10px] sm:text-xs h-7 sm:h-8 px-2 sm:px-3 bg-gray-50 dark:bg-gray-800 hover:bg-gray-100 dark:hover:bg-gray-700 border-gray-200 dark:border-gray-700"
        >
          {action.label}
        </Button>
      ))}
