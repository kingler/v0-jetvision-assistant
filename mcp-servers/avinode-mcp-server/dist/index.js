...
const tools = [
    {
        name: 'search_flights',
        description: 'Search for available charter flights via Avinode API. Returns list of available aircraft from operators.',
        inputSchema: {
            type: 'object',
            properties: {
                departure_airport: {
    ...
    },
    {
        ...
        description: 'Search for empty leg flights (repositioning flights at discounted prices). Can save clients 30-75% on costs.',
        inputSchema: {
            type: 'object',
            properties: {
                departure_airport: {
    ...
    {
        name: 'create_rfp',
        description: 'Create a Request for Proposal (RFP) and send to selected operators on Avinode',
        inputSchema: {
            type: 'object',
            properties: {
                flight_details: {
    ...
    {
        name: 'create_watch',
        description: 'Create a watch to monitor RFP status, price changes, or empty leg availability',
        inputSchema: {
            type: 'object',
            properties: {
                type: {
    ...
    {
        name: 'create_trip',
        description: 'Create a trip container in Avinode and return a deep link for manual operator selection. Supports single-leg, round-trip, and multi-city trips. Use legacy flat params for simple trips or segments[] array for multi-city.',
        inputSchema: {
            type: 'object',
            properties: {
 ...
 */
function normalizeToSegments(params) {
 ...
 */
function determineTripType(segmentCount) {
    if (segmentCount === 1)
        return 'single_leg';
    if (segmentCount === 2)
        return 'round_trip';
...
async function createTrip(params) {
    ...
    const segments = tripSegments.map((seg) => ({
 ...
 */
function transformToRFQFlights(rfqData, tripId) {
    ...
    const quotesFromSellerLift = sellerLifts.flatMap((lift) => {
        ...
        if (lift.links?.quotes && Array.isArray(lift.links.quotes)) {
            ...
            return lift.links.quotes.map((quoteLink) => {
    ...
    [...quotesFromSellerLift, ...quotesFromQuotes, ...quotesFromRequests, ...quotesFromResponses].forEach((quote) => {
    ...
    return quotes.map((quote, index) => {
        ...
        let totalPrice = quote.sellerPrice?.price || // PRIMARY: From fetched quote details (merged at top level)
            ...
            0;
        let currency = quote.sellerPrice?.currency || // PRIMARY: From fetched quote details (merged at top level)
        ...
        const tailPhotoUrl = quote.aircraft?.photos?.[0]?.url ||
...
async function getRFQ(params) {
    ...
    if (isTripId || (!isRfqId && /^[A-Z0-9]+$/.test(apiId))) {
        ...
        try {
            ...
            const tripPayload = tripResponse?.data ?? tripResponse;
            ...
            try {
                ...
                throw new Error(`Failed to fetch RFQs for Trip ID "${params.rfq_id}" (API ID: "${apiId}"): ` +
        ...
        try {
            ...
            // Re-throw with more context
            throw new Error(`Failed to fetch RFQ "${params.rfq_id}" (API ID: "${apiId}"): ${errorMessage}. ` +
    ...
    if (isTripIdResponse) {
        ...
        for (const rfq of rfqs) {
            ...
            const quotes = rfq.quotes ||
                rfq.requests ||
                rfq.responses ||
                (rfq.lifts && Array.isArray(rfq.lifts) ? rfq.lifts.flatMap((lift) => lift.quotes || []) : []) ||
                [];
            ...
            if (hasSellerLift) {
                ...
                if (quoteIds.length > 0) {
                    ...
                    const quoteDetails = await Promise.all(quoteIds.map(async (quoteId) => {
                        try {
                            ...
                            console.log('[getRFQ] Fetched quote detail for', quoteId, ':', {
                                ...
                                hasNestedData: !!quoteResponse?.data?.data,
                        ...
                        for (let i = 0; i < rfq.sellerLift.length; i++) {
                            ...
                            if (lift.links?.quotes && Array.isArray(lift.links.quotes)) {
                                for (let j = 0; j < lift.links.quotes.length; j++) {
                                    ...
                                    const quoteDetail = validQuoteDetails.find((qd) => {
            ...
            const sellerLiftQuoteCount = hasSellerLift
                ? rfq.sellerLift.reduce((count, lift) => {
                    if (lift.links?.quotes && Array.isArray(lift.links.quotes)) {
    ...
    const quotesFromLifts = singleRfqData.lifts && Array.isArray(singleRfqData.lifts)
        ? singleRfqData.lifts.flatMap((lift) => lift.quotes || [])
        : [];
...
async function sendTripMessage(params) {
    ...
    if (params.request_id) {
        try {
            ...
            throw new Error(`Invalid request ID format: "${params.request_id}". ${error instanceof Error ? error.message : String(error)}`);
        }
    }
...
async function getMessage(params) {
    ...
    try {
        ...
        throw new Error(`Invalid message ID format: "${params.message_id}". ${error instanceof Error ? error.message : String(error)}`);
    }
 ...
 */
async function createEmptyLegWatch(params) {
    ...
    try {
        ...
        throw new Error(`Failed to create empty leg watch: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Get all empty leg watches for the current user
 */
async function getEmptyLegWatches(params) {
    ...
    try {
        ...
        const watches = (response.watches || response.data || []).map((w) => ({
            watch_id: w.id || w.watchId,
            status: w.status || 'active',
            departure_airport: w.departureAirport?.icao || w.departure_airport,
            arrival_airport: w.arrivalAirport?.icao || w.arrival_airport,
        ...
        throw new Error(`Failed to get empty leg watches: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Update an empty leg watch
 */
async function updateEmptyLegWatch(params) {
    if (!params.watch_id) {
    ...
    try {
        ...
        if (params.max_price !== undefined)
            updateData.maxPrice = params.max_price;
        if (params.notification_email)
            updateData.notificationEmail = params.notification_email;
        ...
        return {
            ...
            departure_airport: response.departureAirport?.icao,
            arrival_airport: response.arrivalAirport?.icao,
        ...
        throw new Error(`Failed to update empty leg watch: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Delete an empty leg watch
 */
async function deleteEmptyLegWatch(params) {
    if (!params.watch_id) {
        throw new Error('watch_id is required');
    }
    ...
    try {
        ...
        throw new Error(`Failed to delete empty leg watch: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
...
async function getWatchMatches(params) {
    ...
    try {
        ...
        if (params.unviewed_only)
            queryParams.unviewed = true;
        if (params.interested_only)
            queryParams.interested = true;
        const response = await avinodeClient.get(`/emptylegs/watches/${params.watch_id}/matches`, { params: queryParams });
        const matches = (response.matches || response.data || []).map((m) => ({
            match_id: m.id || m.matchId,
            watch_id: params.watch_id,
            empty_leg_id: m.emptyLegId || m.empty_leg_id,
            departure: {
                airport: m.departure?.airport || m.departure_airport,
                name: m.departure?.name,
                city: m.departure?.city,
                date: m.departure?.date || m.departure_date,
                time: m.departure?.time || m.departure_time,
            },
            arrival: {
                airport: m.arrival?.airport || m.arrival_airport,
                name: m.arrival?.name,
                city: m.arrival?.city,
            },
            price: m.price,
            ...
            aircraft: {
                type: m.aircraft?.type,
                model: m.aircraft?.model,
                category: m.aircraft?.category,
                capacity: m.aircraft?.capacity,
                registration: m.aircraft?.registration,
            },
            operator: {
                id: m.operator?.id,
                name: m.operator?.name,
                rating: m.operator?.rating,
            },
        ...
        throw new Error(`Failed to get watch matches: ${error instanceof Error ? error.message : String(error)}`);
    }
}
/**
 * Mark a match as viewed or interested
 */
async function markMatch(params) {
    if (!params.match_id) {
    ...
    try {
        ...
        if (params.viewed !== undefined)
            updateData.viewed = params.viewed;
        if (params.interested !== undefined)
            updateData.interested = params.interested;
        const response = await avinodeClient.patch(`/emptylegs/matches/${params.match_id}`, updateData);
        return {
        ...
        throw new Error(`Failed to mark match: ${error instanceof Error ? error.message : String(error)}`);
    }
}
// Start the server
async function main() {
