...
export class MockAvinodeClient {
  ...
  async post<T = any>(endpoint: string, data: any): Promise<T> {
    ...
    if (endpoint === '/v1/flights/search') {
      return this.mockFlightSearch(data) as T;
    }
  ...
  async put<T = any>(endpoint: string, data: any): Promise<T> {
    ...
    if (endpoint.match(/^\/v1\/trips\/[^/]+\/cancel$/)) {
      const tripId = endpoint.split('/')[3];
      return this.mockCancelTrip(tripId, data) as T;
    }
   ...
   */
  private mockFlightSearch(data: any) {
    const searchId = `SEARCH-${Date.now()}`;
    const mockAircraft = this.generateMockAircraft();

    // Filter by capacity
    const filtered = mockAircraft.filter(
      (a) => a.capacity >= (data.passengers || 1)
    );
   ...
   */
  private mockEmptyLegSearch(data: any) {
   ...
   */
  private mockCreateRFP(data: any) {
    this.tripCounter++;
   ...
   */
  private mockGetRFPStatus(rfpId: string) {
   ...
   * Mock watch creation
   */
  private mockCreateWatch(data: any) {
    return {
   ...
   * Mock airport search
   */
  private mockSearchAirports(query: string) {
    const allAirports = [
   ...
   */
  private mockGetRFQ(rfqId: string) {
   ...
   */
  private mockCreateTrip(data: any) {
    this.tripCounter++;
    const tripId = `atrip-${64956150 + this.tripCounter}`;
   ...
   */
  private mockGetQuote(quoteId: string) {
    const basePrice = Math.floor(Math.random() * 40000) + 30000;
    const taxes = Math.floor(basePrice * 0.075);
    const fees = Math.floor(Math.random() * 2000) + 500;
    const fuelSurcharge = Math.floor(Math.random() * 3000) + 1500;
   ...
   * Mock cancel trip
   */
  private mockCancelTrip(tripId: string, data: any) {
    return {
      trip_id: tripId,
      status: 'cancelled',
   ...
   */
  private mockSendTripMessage(tripId: string, data: any) {
    return {
   ...
   */
  private mockGetTripMessages(tripId: string, params?: { limit?: number; since?: string }) {
    ...
    const mockMessages = [
      ...
      },
      {
        ...
        content:
          'Aircraft confirmed and available. G650 N650NJ is positioned at KTEB and ready for the requested departure.',
